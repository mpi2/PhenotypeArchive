...
public class UnidimensionalChartAndTableProvider {
	...
	public UnidimensionalDataSet doContinuousData(
		...
		for (BiologicalModel biologicalModel : biologicalModels) {
			...
			List<Integer> popIds = unidimensionalStatisticsDAO
			...
			for (Integer popId : popIds) {
				...
				SexType sexType = unidimensionalStatisticsDAO
						.getSexByPopulation(popId);// (new Integer(5959));
				// logger.debug(popId+" sextype="+sexType);
				List<ZygosityType> zygosities = unidimensionalStatisticsDAO
						.getZygositiesByPopulation(popId);
				logger.debug("zygosity for popId=" + zygosities);
				BiologicalModel mutantBiologicalModel = unidimensionalStatisticsDAO
						.getMutantBiologicalModelByPopulation(popId);
				unidimensionalMutantBiologicalModels.add(mutantBiologicalModel);
				BiologicalModel controlBiologicalModel = unidimensionalStatisticsDAO
						.getControlBiologicalModelByPopulation(popId);
				if (i == 0) {// only do this call once as only need once???
					List<UnidimensionalResult> unidimensionalResultList = unidimensionalStatisticsDAO
							.getUnidimensionalResultByParameterAndBiologicalModel(
									parameter, controlBiologicalModel,
									mutantBiologicalModel);
...
//					List<Float> controlCounts = unidimensionalStatisticsDAO
//						.getControlDataPointsForPopulation(popId);
					
					List<Float> controlCounts=new ArrayList<Float>();
					List<MouseDataPoint> controlMouseDataPoints = unidimensionalStatisticsDAO
							.getControlDataPointsWithMouseName(popId);
					mouseDataPointsSet.add(controlMouseDataPoints);
					for(MouseDataPoint mPoint: controlMouseDataPoints){
						controlCounts.add(mPoint.getDataPoint());
					}
...
//							List<Float> mutantCounts = unidimensionalStatisticsDAO
//									.getMutantDataPoints(sexType, zType,
//											parameter, popId);
//							observations2DList.add(mutantCounts);
							List<Float> mutantCounts=new ArrayList<Float>();
							List<MouseDataPoint> mutantMouseDataPoints = unidimensionalStatisticsDAO
									.getMutantDataPointsWithMouseName(sexType, zType,
											parameter, popId);
					...
					if (observations2DList.size() > 1) {// only create the table
														// and graph if there is
														// more than just WT
														// data requested
						String title = parameter.getName();
						ChartData chartAndTable = null;
						if (boxOrScatter.equals(ChartType.UnidimensionalScatter)) {// produce a scatter chart string here
										// rather than an box and scatter plot
										// like below
							chartAndTable = processScatterChartData(title,
									sexType, parameter, zygosities, zyList,
									mouseDataPointsSet, mutantBiologicalModel);

						} else {
							chartAndTable = processChartData(title, sexType,
									parameter, zygosities, zyList,
									observations2DList, mutantBiologicalModel);

						}
						// return an new class to represent a unidimensional
						// data set chart and table object to include other
	...
	private ChartData processScatterChartData(String title, SexType sexType,
		...
		for (ZygosityType zType : zygosities) {
			if (zyList.isEmpty() || zyList.contains(zType.name())) {
				categoriesList.add(zType.name().substring(0, 3)
						.toUpperCase());
				String alleleComposition = mutantBiologicalModel
						.getAllelicComposition();
				if (zType.equals(ZygosityType.homozygote)) {// if homozygote
		...
		// other column
		//for each set of raw data  0 being control data and 1 being mutant for specific gender we want a set of value pairs for x and y axis
		for (List<MouseDataPoint> listOfFloats : rawData) {
		...
		String chartString = createScatterPlotChartsString(
				categoriesList, title, sexType, yAxisTitle,
				theoreticalMean, null, scatterColumns);
		// continuousCharts.add(chartString);
		ChartData cNTable = new ChartData();
		// cNTable.setTable(table);
		cNTable.setChart(chartString);
...
//				+ ", marker: { fillColor: 'white', lineWidth: 1, lineColor: Highcharts.getOptions().colors[0] }, tooltip: { pointFormat: '{point.y:..4f}' }          }] }); }";
		...
		String scatterChartString = "{ chart: { type: 'scatter', zoomType: 'xy' }, title: { text: '"+title+"' }, subtitle: { text: '"+WordUtils.capitalize(sex.name())+"' }, xAxis: { title: { enabled: true, text: '"+xAxisTitle+"' },  showLastLabel: true }, yAxis: { title: { text: '"+yAxisTitle+"' } }, legend: { layout: 'vertical', align: 'left', verticalAlign: 'top', x: 100, y: 70, floating: true, backgroundColor: '#FFFFFF', borderWidth: 1 }, plotOptions: { scatter: { marker: { radius: 5, states: { hover: { enabled: true, lineColor: 'rgb(100,100,100)' } } }, states: { hover: { marker: { enabled: false } } }, tooltip: { headerFormat: '<b>{series.name}</b><br>', pointFormat: 'mouse {point.x} , {point.y}"+yAxisTitle+"' } } },"
				+ " series: [{ name: '"+xAxisCategoriesList.get(0)+"', color: 'rgba(223, 83, 83, .5)', "
				+ "data:" + controlScatterString + "}, " + // end of female
				"{ name: '"+xAxisCategoriesList.get(1)+"', color: 'rgba(119, 152, 191, .5)', data:" +
				mutantScatterString +
				" }" +//end of male
				"] " + // end of series
				"}); }";
