...
public class ExperimentService {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(ExperimentService.class);
	...
	public List<ExperimentDTO> getExperimentDTO(Integer parameterId,
			Integer pipelineId, String geneAccession, SexType sex,
			Integer phenotypingCenterId, List<String> zygosity, String strain)
			throws SolrServerException, IOException, URISyntaxException {
		return getExperimentDTO(parameterId, pipelineId, geneAccession, sex,
				phenotypingCenterId, zygosity, strain, null, Boolean.TRUE);
	}
	...
	public List<ExperimentDTO> getExperimentDTO(Integer parameterId,
		...
		List<ObservationDTO> observations = os
				.getExperimentalUnidimensionalObservationsByParameterPipelineGeneAccZygosityOrganisationStrainSexSexAndMetaDataGroup(
						...
						phenotypingCenterId, strain, sex, metaDataGroup);
		...
		for (ObservationDTO observation : observations) {
			...
			String experimentKey = observation.getPhenotypingCenter()
					...
					+ observation.getGeneAccession() // TODO: should this be
														// alleleAccession?
			...
			if (experimentsMap.containsKey(experimentKey)) {
				...
				experiment.setObservationType(ObservationType
						.valueOf(observation.getObservationType()));
				experiment.setHomozygoteMutants(new HashSet<ObservationDTO>());
				experiment
						.setHeterozygoteMutants(new HashSet<ObservationDTO>());
			...
			if (experiment.getParameterStableId() == null) {
				experiment.setParameterStableId(observation
			...
			if (experiment.getExperimentalBiologicalModelId() == null) {
				experiment.setExperimentalBiologicalModelId(observation
						.getBiologicalModelId());
			}

			experiment.getZygosities().add(
					ZygosityType.valueOf(observation.getZygosity()));
			...
			if (experiment.getResults() == null
					&& experiment.getExperimentalBiologicalModelId() != null
					&& includeResults) {
				...
				// for unidimensional
				List<? extends StatisticalResult> basicResults = phenoDAO
						.getStatisticalResultFor(
								observation.getGeneAccession(), observation
										.getParameterStableId(),
					...
					for (StatisticalResult basicResult : basicResults) {
						...
						try {
							...
							if (result != null) {
								...
								} else if (metaDataGroup.equals(result.getMetadataGroup())) {
									populatedResults.add(result);
								}
							}
			...
			if (ZygosityType.valueOf(observation.getZygosity()).equals(
					ZygosityType.heterozygote)) {
				experiment.getHeterozygoteMutants().add(observation);
			} else if (ZygosityType.valueOf(observation.getZygosity()).equals(
					ZygosityType.homozygote)) {
				experiment.getHomozygoteMutants().add(observation);
			} else if (ZygosityType.valueOf(observation.getZygosity()).equals(
					ZygosityType.hemizygote)) {
		...
		for (String key : experimentsMap.keySet()) {
			...
			// organisationId
			// parameter will not be null and we can use that, otherwise we need
			// to
			...
			if (experiment.getControls() == null) {
				...
				if (experiment.getObservationType().equals(
					...
					experiment
							.setControlSelectionStrategy(ControlStrategy.baseline_all);
					...
					if (experiment.getSexes() != null) {
						...
						for (SexType s : SexType.values()) {
							...
							controls.addAll(os.getAllControlsBySex(parameterId,
									experiment.getStrain(),
									experimentOrganisationId, null,
									s.toString(), experiment.getMetadataGroup()));
						...
						controls.addAll(os.getAllControlsBySex(parameterId,
								experiment.getStrain(),
								experimentOrganisationId, null, null,
					...
					for (ObservationDTO o : experiment.getMutants()) {
						...
						allBatches.add(o.getDateOfExperiment().getYear() + "-"
								+ o.getDateOfExperiment().getMonth() + "-"
								+ o.getDateOfExperiment().getDate());

						if (experimentOrganisationId == null) {
							experimentOrganisationId = o
									.getPhenotypingCenterId();
						}
					...
					if (experiment.getSexes() != null
						...
						for (SexType s : SexType.values()) {
							...
							if (allBatches.size() == 1) {
								...
								List<ObservationDTO> potentialControls = os
										.getConcurrentControlsBySex(
												parameterId,
												experiment.getStrain(),
												experimentOrganisationId,
												experimentDate, s.name(),
												experiment.getMetadataGroup());
								LOG.info("Number of potential controls for sex: "
										+ s.name()
										+ ": "
										+ potentialControls.size());

								if (potentialControls.size() >= MIN_CONTROLS) {

									controls = potentialControls;
									experiment
											.setControlSelectionStrategy(ControlStrategy.concurrent);
									LOG.info("Setting concurrent controls for sex: "
											+ s.name());

								} else {

									controls = os.getAllControlsBySex(
											parameterId,
											experiment.getStrain(),
											experimentOrganisationId, null,
											s.name(),
											experiment.getMetadataGroup());
									LOG.info("Using baseline controls for sex: "
											+ s.name()
											+ ", num controls: "
											+ controls.size());
								...
								controls = os
										.getAllControlsBySex(parameterId,
												experiment.getStrain(),
												experimentOrganisationId, null,
												s.name(),
												experiment.getMetadataGroup());
						...
						if (allBatches.size() == 1) {
							...
							List<ObservationDTO> potentialMaleControls = os
									.getConcurrentControlsBySex(parameterId,
											experiment.getStrain(),
											experimentOrganisationId,
											experimentDate,
											SexType.male.name(),
											experiment.getMetadataGroup());
							List<ObservationDTO> potentialFemaleControls = os
									.getConcurrentControlsBySex(parameterId,
											experiment.getStrain(),
											experimentOrganisationId,
											experimentDate,
							...
							LOG.info("Number of potential controls for males: "
									+ potentialMaleControls.size());
							LOG.info("Number of potential controls for females: "
									+ potentialFemaleControls.size());
							...
							if (potentialMaleControls.size() >= MIN_CONTROLS
									&& potentialFemaleControls.size() >= MIN_CONTROLS) {
								...
								controls = os.getAllControlsBySex(parameterId,
										experiment.getStrain(),
										experimentOrganisationId, null, null,
										experiment.getMetadataGroup());
								LOG.info("Using baseline controls, num controls: "
										+ controls.size());
				...
				if (experiment.getControlBiologicalModelId() == null
						&& controls.size() > 0) {
					experiment.setControlBiologicalModelId(controls.get(0)
							.getBiologicalModelId());
				}
	 ...
	 */
	public List<ExperimentDTO> getExperimentDTO(Integer parameterId,
			Integer pipelineId, String geneAccession)
			throws SolrServerException, IOException, URISyntaxException {
		return getExperimentDTO(parameterId, pipelineId, geneAccession, null,
				null, null, null);
	}

	public List<ExperimentDTO> getExperimentDTO(String parameterStableId,
			Integer pipelineId, String geneAccession)
			throws SolrServerException, IOException, URISyntaxException {
	...
	public List<ExperimentDTO> getExperimentDTO(String parameterStableId,
			Integer pipelineId, String geneAccession, String strain)
			throws SolrServerException, IOException, URISyntaxException {
		Parameter p = parameterDAO.getParameterByStableId(parameterStableId);
		return getExperimentDTO(p.getId(), pipelineId, geneAccession, null,
				null, null, strain);
	}
	...
	public List<ExperimentDTO> getExperimentDTO(String parameterStableId,
		...
		return getExperimentDTO(p.getId(), pipelineId, geneAccession, sex,
				phenotypingCenterId, zygosity, strain);
	}
	 ...
	 */
	public ExperimentDTO getSpecificExperimentDTO(Integer id,
			Integer pipelineId, String acc, List<String> genderList,
			List<String> zyList, Integer phenotypingCenterId, String strain,
			String metadataGroup) throws SolrServerException, IOException,
			URISyntaxException, SpecificExperimentException {
		List<ExperimentDTO> experimentList = new ArrayList<ExperimentDTO>();
		boolean includeResults = true;
		if (genderList.isEmpty() || genderList.size() == 2) {// if gender list
																// is size 2
																// assume both
																// sexes so no
																// filter needed

			if (zyList.isEmpty() || zyList.size() == 3) {// if zygosity list is
															// size 2 then no
															// filter needed
															// either
				experimentList = this.getExperimentDTO(id, pipelineId, acc,
						null, phenotypingCenterId, null, strain, metadataGroup,
						includeResults);
			} else {
				experimentList = this.getExperimentDTO(id, pipelineId, acc,
						null, phenotypingCenterId, zyList, strain,
						metadataGroup, includeResults);
			}
			...
			if (zyList.isEmpty() || zyList.size() == 3) {
				experimentList = this.getExperimentDTO(id, pipelineId, acc,
						SexType.valueOf(gender), phenotypingCenterId, null,
						strain, metadataGroup, includeResults);
			} else {
				experimentList = this.getExperimentDTO(id, pipelineId, acc,
						SexType.valueOf(gender), phenotypingCenterId, zyList,
						strain, metadataGroup, includeResults);
			}
		...
		if (experimentList.size() > 1) {
			throw new SpecificExperimentException(
					"too many experiments returned - should only be one from this method call");
		}
		return experimentList.get(0);
	}

	public Map<String, List<String>> getExperimentKeys(String mgiAccession,
			String parameterStableIds, List<String> pipelineStableIds,
			List<String> phenotypingCenter, List<String> strain,
			List<String> metaDataGroup) throws SolrServerException {
		return os.getExperimentKeys(mgiAccession, parameterStableIds,
				pipelineStableIds, phenotypingCenter, strain, metaDataGroup);
	}
	 ...
	 */
	public ControlSelectionStrategy getControlSelectionStrategy(
			String[] phenotypingCenter, String[] strategies) {
