            ...
            <table class="globalTest">
                ...
                <tbody>
                    <c:choose>
                        <c:when test="${data.result.significanceClassification.text == 'Both genders equally' || data.result.significanceClassification.text == 'No significant change'  || data.result.significanceClassification.text == 'Can not differentiate genders' }">
                            <tr>
                                <td class="globalTestValue">${data.result.nullTestSignificance}</td>
                                <td>${data.result.significanceClassification.text}</td>
                                <td>${data.result.genotypeParameterEstimate}</td>
                            </tr>
                        </c:when>
                        <c:when test="${data.result.significanceClassification.text == 'Female only' || data.result.significanceClassification.text == 'Male only'  || data.result.significanceClassification.text == 'Different size females greater' || data.result.significanceClassification.text == 'Different size males greater' || data.result.significanceClassification.text == 'Female and male different directions'}">
                            <tr>
                                ...
                                <td>Female</td>
                                <td class="effect">${data.result.genderFemaleKoEstimate}</td>
                                <c:if test="${data.result.genderFemaleKoStandardErrorEstimate!=null}">
                                    <td>&#177;${data.result.genderFemaleKoStandardErrorEstimate }</td>
                                </c:if>
                                <c:if test="${data.result.genderFemaleKoPValue!=null}">
                                    <td class="pvalue">${data.result.genderFemaleKoPValue }</td>
                                </c:if>
                            </tr>
                            <tr>
                                <td> Male</td>
                                <td class="effect"> ${data.result.genderMaleKoEstimate}</td>
                                <c:if test="${data.result.genderMaleKoStandardErrorEstimate!=null}">
                                    <td>&#177;${data.result.genderMaleKoStandardErrorEstimate }</td>
                                </c:if>
                                <c:if test="${data.result.genderMaleKoPValue!=null}">
                                    <td class="pvalue">${data.result.genderMaleKoPValue }</td>
                                </c:if>
                            </tr>
                        </c:when>
                    </c:choose>
    ...
    <script>
        $(document).ready(
                function() {
                 ...
                 $("#toggle_table${experimentNumber}").hide();//hide on lo   ad
                 $("#toggle_table_button${experimentNumber}").toggleClass('fa-caret-right');//toggle the arrow on the link to point right as should be closed on in   it
                    $("#toggle_table_button${experimentNumber}").click(function() {
                     console.log("cli   ck fired");
                     $("#toggle_table${experimentNumber}").togg   le('slow');
